...
export const POSSystem = () => {
  ...
  return (
    <div className="flex-1 flex flex-col lg:flex-row overflow-hidden bg-[#F8FAFC]">
      ...
      <div className="flex-1 flex flex-col min-w-0 p-4 lg:p-6 border-r border-slate-100">
        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredProducts.map((product) => (
              ...
              >
                <Card className="border border-slate-100 shadow-sm hover:shadow-md transition-all duration-300 rounded-xl overflow-hidden bg-white">
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-bold text-primary">₹{product.salePrice.toLocaleString()}</p>
                      <Badge variant="outline" className="text-[8px] font-bold uppercase tracking-tighter border-slate-100 bg-slate-50 text-slate-400">
      ...
      <div className="w-full lg:w-[400px] flex flex-col bg-white shadow-2xl shadow-slate-200/50 z-10">
        <div className="p-6 border-b border-slate-100 bg-slate-50/50">
          ...
          <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mt-1">Transaction Summary & Payment</p>
        </div>
        ...
        <div className="p-6 space-y-4 border-b border-slate-100">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="text-[9px] font-bold uppercase tracking-widest text-slate-400 px-1">Customer Name</label>
              <div className="relative">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 w-3 h-3 text-slate-400" />
                <input 
                  type="text" 
                  placeholder="Full Name"
                  className="w-full h-9 pl-8 pr-3 text-xs font-semibold bg-slate-100 border-none rounded-lg focus:ring-1 focus:ring-primary/20 outline-none"
                  value={customer.name}
            ...
            <div className="space-y-1.5">
              ...
              <input 
                ...
                className="w-full h-9 px-3 text-xs font-semibold bg-slate-100 border-none rounded-lg focus:ring-1 focus:ring-primary/20 outline-none"
            ...
            {cart.length === 0 ? (
              ...
              cart.map((item) => (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: -20 }}
                  className="group relative bg-slate-50/50 hover:bg-slate-50 rounded-xl p-3 border border-slate-100 transition-colors"
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex-1 pr-4">
                      <h5 className="text-[11px] font-bold text-slate-900 leading-tight uppercase">{item.name}</h5>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1 bg-white border border-slate-100 rounded-xl p-0.5">
                      <button 
                      ...
                      <span className="w-8 text-center text-[11px] font-semibold">{item.qty}</span>
                      <button 
                        onClick={() => updateQty(item.id, 1)}
                    ...
                    <p className="text-xs font-bold text-slate-900 tracking-tight">₹{(item.price * item.qty).toLocaleString()}</p>
                  </div>
                </motion.div>
              ))
            )}
        ...
        <div className="p-6 bg-slate-900 rounded-t-2xl">
          <div className="space-y-2 mb-4">
            ...
            <div className="flex justify-between items-center">
              <span className="text-[11px] font-bold text-white uppercase tracking-widest">Total Amount</span>
              <span className="text-2xl font-bold text-white tracking-tight">₹{totals.total.toLocaleString()}</span>
            </div>
          </div>
          ...
          <Button 
            className="w-full h-12 bg-white hover:bg-slate-100 text-slate-900 font-bold uppercase tracking-widest rounded-xl shadow-xl transition-all active:scale-95 disabled:opacity-50 group"
